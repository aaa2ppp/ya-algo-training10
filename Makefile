...
lib/contestio:
	mkdir -p lib
	cd lib && git clone git@github.com:aaa2ppp/contestio.git
	cd lib/contestio && git checkout satellite

local: lib/contestio ## switch to local contestio library (creates go.work)
	@set -e; \
	test -d "$(LOCAL_LIB)"; \
